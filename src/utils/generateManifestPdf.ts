...
export function generateManifestPDF(shipment: Shipment, piecesList: any[]) {
  ...
  doc.text('Planta de Fabricación de Estructuras Metálicas • Parque Industrial Nave 4', margin + 16, 21.5);
  doc.text('Certificación AISC & AWS D1.1 • Tel: +504 2550-0000 • www.alanzaestructuras.com', margin + 16, 25.5);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(100, 110, 125);
  doc.text('EMPRESA TRANSPORTISTA:', margin + colWidth + 4, gridY + 5.5);
  doc.setFontSize(8.5);
  doc.setTextColor(21, 28, 39);
  doc.text(shipment.carrier || 'Logística Rápida SA', margin + colWidth + 4, gridY + 10.5);
  doc.setFontSize(7.5);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(85, 95, 111);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(100, 110, 125);
  doc.text('UNIDAD DE TRANSPORTE:', margin + colWidth * 2 + 4, gridY + 5.5);
  doc.setFontSize(8.5);
  doc.setTextColor(21, 28, 39);
  doc.setFont('courier', 'bold');
  doc.text(`Placas: ${shipment.truckPlates || 'HN-8842-TR'}`, margin + colWidth * 2 + 4, gridY + 10.5);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.setTextColor(85, 95, 111);
  doc.text('Plataforma 40 pies', margin + colWidth * 2 + 4, gridY + 15);
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Certificación de Conformidad & Transporte:', margin + 3, noteY + 4.5);
  doc.setFont('helvetica', 'normal');
  const disclaimerText = 'Los elementos amparados en este documento han sido fabricados e inspeccionados conforme a las especificaciones técnicas aprobadas y la norma AWS D1.1 / AISC 360. El transportista declara recibir la carga asegurada e inventariada conforme para su traslado.';
  ...
  doc.text('Juan Pérez (Jefe Planta)', margin + sigColWidth / 2, sigY + 10, { align: 'center' });
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(21, 28, 39);
  doc.text('Despachado por (Planta)', margin + sigColWidth / 2, sigY + 16, { align: 'center' });
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7);
  doc.setTextColor(100, 116, 139);
  ...
  doc.setFontSize(8);
  doc.setTextColor(21, 28, 39);
  doc.text('Conductor / Transportista', margin + sigColWidth * 1.5, sigY + 16, { align: 'center' });
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7);
  doc.setTextColor(100, 116, 139);
  doc.text(shipment.carrier || 'Logística Rápida SA', margin + sigColWidth * 1.5, sigY + 20, { align: 'center' });
