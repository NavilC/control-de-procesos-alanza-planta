...
export const ImportPiecesModal: React.FC = () => {
  ...
  const initializeDataset = (rawRows: Record<string, any>[], sourceName: string) => {
    if (!rawRows || rawRows.length === 0) {
      showToast('El archivo no contiene filas con datos legibles.', 'error');
  ...
  const processWorkbook = (workbook: XLSX.WorkBook, uploadedName: string) => {
    ...
    if (!sheet) {
      showToast('El archivo Excel no contiene hojas válidas.', 'error');
      playFeedbackSound('error');
      return;
    }
    ...
    if (!rawMatrix || rawMatrix.length === 0) {
      showToast('El archivo Excel seleccionado está vacío.', 'warning');
    ...
    if (rawObjects.length === 0) {
      showToast('No se encontraron filas con datos de piezas bajo los encabezados detectados.', 'warning');
      playFeedbackSound('warning');
      return;
    }
  ...
  const handleFileUpload = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        showToast('Ocurrió un error al leer el archivo. Verifica que sea un Excel (.xlsx, .xls) o CSV válido.', 'error');
        playFeedbackSound('error');
      }
    };
  ...
  const handleProcessPastedJson = () => {
    if (!jsonText.trim()) {
      showToast('Por favor pega un JSON o contenido de texto.', 'warning');
      playFeedbackSound('warning');
      return;
    }
    ...
    try {
      ...
      showToast('No se pudo interpretar el texto. Asegúrate de que sea un JSON válido o texto tabulado de Excel.', 'error');
      playFeedbackSound('error');
    }
  };
  ...
  const handleConfirmImport = () => {
    ...
    if (selectedRows.length === 0) {
      showToast('Por favor selecciona al menos una pieza para importar.', 'warning');
      playFeedbackSound('warning');
      return;
    }
