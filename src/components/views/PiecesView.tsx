...
export const PiecesView: React.FC = () => {
  ...
  const [paseFilter, setPaseFilter] = useState<'all' | 'with-pass' | 'without-pass'>('all');
  ...
  const filteredPieces = pieces.filter(p => {
    ...
    // Check Pase de Salida condition
    const hasPase = Boolean(p.refId?.startsWith('ENV-') || findShipmentForPiece(p, shipments));
    const matchesPase = 
      paseFilter === 'all' ? true :
      paseFilter === 'with-pass' ? hasPase :
      !hasPase;

    return matchesSearch && matchesProject && matchesStatus && matchesType && matchesPase;
  });
    ...
    <div className="space-y-5 pb-16 animate-in fade-in duration-200 relative">
      ...
      {selectedPieces.length > 0 && (
        <div className="sticky top-20 z-20 bg-[#151c27] text-white rounded-xl p-3.5 px-4 shadow-xl border border-slate-700 flex flex-wrap items-center justify-between gap-3 animate-in slide-in-from-top-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => {
                const selected = pieces.filter(p => selectedPieces.includes(p.id));
                playFeedbackSound('click');
                setSelectedPiecesForBatchQR(selected);
              }}
              className="px-3 py-1.5 bg-blue-700 hover:bg-blue-600 text-white rounded-lg text-[12px] font-bold flex items-center gap-1.5 transition-colors shadow-xs"
              title="Generar e imprimir etiquetas QR para las piezas seleccionadas"
            >
              <QrCode className="w-3.5 h-3.5 text-blue-200" />
              <span>Etiquetas QR ({selectedPieces.length})</span>
            </button>
      ...
      <div className="bg-white rounded-xl border border-[#c3c6d7] p-3.5 sm:p-4 flex flex-wrap gap-3 items-center shadow-xs">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <select
            value={paseFilter}
            onChange={(e) => setPaseFilter(e.target.value as any)}
            className="h-10 px-3 rounded-lg bg-white border border-[#c3c6d7] text-[13px] text-[#151c27] focus:border-[#004ac6] outline-none cursor-pointer min-w-[145px]"
            title="Filtrar según si la pieza tiene pase de salida activo o está en taller"
          >
            <option value="all">Pase (Todos)</option>
            <option value="with-pass">En Pase de Salida</option>
            <option value="without-pass">Sin Pase (En Planta)</option>
          </select>
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1.5 text-[12px] bg-[#f9fafb] p-2.5 rounded-lg border border-[#e2e8f8] mb-3">
                  ...
                  {(() => {
                    const matchedShipment = findShipmentForPiece(piece, shipments);
                    const isPass = Boolean(piece.refId?.startsWith('ENV-') || matchedShipment);
                    const passId = matchedShipment?.id || piece.refId;
                    return (
                      <div className="flex justify-between items-center pt-1 border-t border-[#edf0f7]">
                        <span className="text-[#737686]">Pase:</span>
                        {isPass && passId !== '-' ? (
                          <span
                            onClick={(e) => {
                              e.stopPropagation();
                              playFeedbackSound('click');
                              if (matchedShipment) setSelectedShipmentForPrint(matchedShipment);
                              else setSelectedPieceForDetail(piece);
                            }}
                            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-mono font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 cursor-pointer transition-colors"
                            title="Ver pase de salida"
                          >
                            <Truck className="w-3 h-3 text-emerald-600" />
                            {passId}
                          </span>
                        ) : (
                          <span className="text-[#737686] text-[11px] italic">En Taller</span>
                        )}
                      </div>
                    );
                  })()}
                ...
                <div className="flex items-center justify-between pt-2 border-t border-[#e2e8f8] text-[11px]">
                  <div className="flex items-center gap-1.5">
                    ...
                    {piece.modificationHistory && piece.modificationHistory.length > 0 && (
                      <button
                        ...
                        className="text-amber-800 font-semibold hover:underline flex items-center gap-0.5 bg-amber-50 px-1 py-0.5 rounded border border-amber-200 text-[10px]"
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="px-2 py-0.5 bg-slate-900 hover:bg-slate-800 text-white rounded text-[11px] font-bold flex items-center gap-1 transition-colors shadow-2xs"
                    ...
                    >
                      ...
                      <td className={`${viewMode === 'compact' ? 'py-1.5 px-2.5' : 'py-2.5 px-3.5'} font-mono font-medium`}>
                        {(() => {
                          const matchedShipment = findShipmentForPiece(piece, shipments);
                          const isPass = Boolean(piece.refId?.startsWith('ENV-') || matchedShipment);
                          const passDisplay = matchedShipment?.id || piece.refId;

                          if (isPass && passDisplay !== '-') {
                            return (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-mono font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 cursor-pointer transition-colors shadow-2xs"
                                title="Pieza en Pase de Salida. Clic para ver manifiesto de despacho."
                              >
                          ...
                          return <span className="text-[#737686] text-[11px] italic">En Planta</span>;
                        })()}
                      </td>
                      <td className={`${viewMode === 'compact' ? 'py-1.5 px-2.5' : 'py-2 px-3.5'} text-right`}>
                        <div className="flex items-center justify-end gap-1.5">
                          {/* Botón QR */}
                          <button
                            id={`qr-piece-btn-${piece.id}`}
                            onClick={(e) => {
                              e.stopPropagation();
                              playFeedbackSound('click');
                              setSelectedPieceForQR(piece);
                            }}
                            className="px-2 py-1 bg-slate-900 hover:bg-slate-800 text-white rounded-md text-[12px] font-bold flex items-center gap-1 transition-colors shadow-2xs cursor-pointer"
                            title="Ver e imprimir código QR y etiqueta de la pieza"
                          >
