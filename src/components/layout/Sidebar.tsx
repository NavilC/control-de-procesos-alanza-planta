...
export const Sidebar: React.FC<{ isOpen?: boolean; onClose?: () => void }> = ({ isOpen: propIsOpen, onClose: propOnClose }) => {
  const { 
    activeTab, 
    setActiveTab, 
    incidents, 
  ...
  const handleClose = propOnClose || (() => setIsSidebarOpen(false));
  ...
  const NAV_ITEMS: NavItem[] = [
    ...
    { id: 'esquema', label: 'Esquema de Montaje', iconName: 'grid_view', LucideIcon: Grid3X3 },
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed md:relative left-0 top-0 h-full bg-[#2a313d] text-[#ebf1ff] border-r border-[#3d4756] flex flex-col z-50 md:z-20 transition-all duration-300 ease-in-out shrink-0 overflow-hidden ${
          isOpen 
            ? 'translate-x-0 w-[260px] opacity-100 shadow-xl md:shadow-none' 
            : '-translate-x-full md:translate-x-0 w-[260px] md:w-0 md:border-r-0 opacity-0 md:opacity-0 pointer-events-none'
        }`}
        id="sidebar"
        aria-hidden={!isOpen}
      >
        <div className="w-[260px] h-full flex flex-col py-4 shrink-0">
          {/* Brand Header */}
          <div className="px-4 mb-6 flex items-center justify-between gap-2">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="overflow-hidden">
                <h1 className="text-[14px] font-bold text-white tracking-tight leading-tight uppercase font-['Inter'] truncate">
                  CONTROL DE ENVÍOS
                </h1>
                <p className="text-[10px] text-[#b4c5ff] font-semibold tracking-wider uppercase mt-0.5 opacity-90 truncate">
                  ALANZA CONSTRUCCION
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="p-1.5 rounded-lg text-[#94a3b8] hover:text-white hover:bg-[#3d4756] transition-colors shrink-0"
