...
export const TopNavBar: React.FC<TopNavBarProps> = ({ onToggleSidebar = () => {} }) => {
  ...
  return (
    <>
      <header className="fixed top-0 right-0 left-0 md:left-[260px] z-30 h-16 bg-[#ffffff] border-b border-[#e2e8f8] flex items-center justify-between px-3 md:px-6 transition-all duration-200">
        {/* Left: Mobile Toggle & Global Search */}
        <div className="flex items-center gap-2 md:gap-3 flex-1 max-w-lg">
          <button 
            onClick={onToggleSidebar}
            className="md:hidden p-2 rounded-lg text-[#434655] hover:bg-[#f0f3ff] transition-colors"
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="relative">
            ...
            >
              <div className="w-8 h-8 rounded-full bg-[#2563eb] text-white flex items-center justify-center font-bold text-[12px] shadow-sm">
                {currentUserRole === 'Administrador' ? 'JP' : currentUserRole === 'Producción' ? 'RM' : 'AG'}
              </div>
              <div className="hidden sm:flex flex-col items-start leading-tight">
                <span className="text-[13px] font-semibold text-[#151c27]">
                  {currentUserRole === 'Administrador' ? 'Juan Pérez' : currentUserRole === 'Producción' ? 'Ing. Roberto M.' : 'Ing. Ana Gómez'}
                </span>
              ...
              <div className="absolute right-0 mt-2 w-72 bg-white border border-[#c3c6d7] rounded-xl shadow-xl z-50 p-2 text-[13px] animate-in fade-in">
                <div className="p-2.5 border-b border-[#e2e8f8] mb-1">
                  <p className="font-semibold text-[#151c27]">
                    {currentUserRole === 'Administrador' ? 'Juan Pérez' : currentUserRole === 'Producción' ? 'Ing. Roberto Mendoza' : 'Ing. Ana Gómez'}
                  </p>
                  <p className="text-[11px] text-[#737686]">
                    {currentUserRole === 'Administrador' ? 'jperez@alanza.com' : currentUserRole === 'Producción' ? 'rmendoza@alanza.com' : 'agomez@alanza.com'}
                  </p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl max-w-sm w-full border border-[#c3c6d7] shadow-2xl p-6 text-center animate-in zoom-in-95">
            ...
            <p className="text-[13px] text-[#555f6f] mt-1.5 mb-5 leading-relaxed">
              Finalizarás la sesión activa de <strong>{currentUserRole === 'Administrador' ? 'Juan Pérez' : currentUserRole === 'Producción' ? 'Ing. Roberto Mendoza' : 'Ing. Ana Gómez'}</strong>. Se guardarán todos tus cambios y podrás volver a ingresar en cualquier momento.
            </p>
            ...
            <div className="p-6 space-y-4 text-[13px] text-[#434655] max-h-[70vh] overflow-y-auto">
              <div className="p-3 bg-[#f0f3ff] rounded-lg border border-[#dce2f3]">
                ...
                <p>
                  ...
                  3. <strong>Envíos:</strong> Agrupación en remisión de carga y asignación de transportista.<br/>
