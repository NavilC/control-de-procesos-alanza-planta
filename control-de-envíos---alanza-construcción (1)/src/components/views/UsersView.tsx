...
export const UsersView: React.FC = () => {
  const { addAudit, playFeedbackSound } = useApp();

  const [usersList, setUsersList] = useState<UserData[]>([
    {
      id: 'usr-1',
      name: 'Carlos Gómez',
      email: 'carlos.gomez@alanza.com',
    ...
    },
    {
      id: 'usr-3',
      name: 'Juan Pérez',
      email: 'juan.perez@alanza.com',
      role: 'Residente de campo',
      projects: ['Mhotivo'],
      lastActive: 'Hoy, 09:15',
      status: 'Activo',
    ...
    },
    {
      ...
      email: 'roberto.mendoza@alanza.com',
    ...
    },
    {
      ...
      email: 'manuel.hernandez@alanza.com',
      role: 'Bodeguero',
    ...
    },
    {
      id: 'usr-6',
      name: 'Laura Morales',
      email: 'laura.morales@alanza.com',
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    addAudit('Creación de Usuario', 'Usuarios y Roles', `Usuario creado: ${newUser.name} (${newUser.email}) con rol "${newUser.role}"`);
    playFeedbackSound('success');

    setSuccessMessage(`Usuario "${newUser.name}" creado con rol "${newUser.role}"`);
  ...
  const handleToggleStatus = (id: string) => {
    setUsersList(prev => prev.map(u => {
      if (u.id === id) {
        ...
        addAudit('Modificación Usuario', 'Usuarios y Roles', `Estado de ${u.name} cambiado a ${nextStatus}`);
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full border border-[#c3c6d7] p-6 space-y-4 shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleCreateUser} className="space-y-4 text-[13px]">
              ...
              <div>
                <label className="block font-semibold text-[#434655] mb-1">Correo Electrónico Corporativo *</label>
                <input
                  required
                  type="email"
                  value={newEmail}
                  onChange={(e) => setNewEmail(e.target.value)}
                  placeholder="laura.mendoza@alanza.com"
                  className="w-full h-10 px-3 bg-[#f0f3ff] border border-[#c3c6d7] rounded-lg outline-none focus:border-[#004ac6] focus:bg-white transition-colors"
                />
              </div>
